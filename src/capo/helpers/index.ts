...
export function hasRel(attributes: Attributes, expectedToken: string): boolean {
  const rel = attrLower(attributes, 'rel')
  if (!rel) return false
...
export function hasType(attributes: Attributes, expected: string): boolean {
  return attrLower(attributes, 'type') === expected
}

export function hasSrc(attributes: Attributes): boolean {
  return hasAttr(attributes, 'src') && (attributes.src ?? '') !== ''
}

export function hasBlockingRender(attributes: Attributes): boolean {
  return attrLower(attributes, 'blocking') === 'render'
}

export function isBooleanAttrPresent(attributes: Attributes, key: string): boolean {
